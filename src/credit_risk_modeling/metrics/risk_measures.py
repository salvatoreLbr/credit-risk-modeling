...
def compute_risk_measures_simulation(m: int, loss_distribution: np.array, alpha: float) -> Tuple[float]:
    """
...
def compute_risk_measure_analytic(N: int, alpha: float, pmf: np.array, cdf: np.array, c: Union[np.array, float]):
    """
